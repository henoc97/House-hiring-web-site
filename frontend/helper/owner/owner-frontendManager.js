...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (recentTenantsTable) {
    ...
    searchInput.addEventListener('input', function () {
      this.value !== ''
        ? getUnvalidReceiptsRequest(true)
        : getUnvalidReceiptsRequest(false);
  ...
  const unValidReceiptsTable = document.getElementById(
    'unvalid-receipts-table'
  );
    ...
    link.addEventListener('click', function (e) {
      ...
      if (this.id === 'dash-button') {
        fetch(ownerURL + '/dashboard')
          ...
          .then((data) => {
            ...
            const recentTenantsTable = document.getElementById(
      ...
      if (this.id === 'proprietes-button') {
        fetch(ownerURL + '/properties')
          ...
          .then((data) => {
            ...
            const propertiesTable = document.getElementById(
              'my-properties-table'
            );
            if (propertyForm && propertiesTable) {
              ...
              searchInput.addEventListener('input', function () {
                this.value !== ''
                  ? getPropertiesRequest(1, true) // Search tenants
        ...
        fetch(ownerURL + '/tenants')
          ...
          .then((data) => {
            ...
            if (tenantsTable && searchInput) {
              ...
              searchInput.addEventListener('input', function () {
                this.value !== ''
                  ? getAllTenantsRequest(true) // Search tenants
                  : getAllTenantsRequest(false); // Fetch all tenants
              });
            }
            if (editTenantForm) {
              editTenantForm.addEventListener('submit', function (event) {
                event.preventDefault(); // Prevent page reload
                const submitButton = event.target.querySelector(
                  '#edit-tenant-form button[type="submit"]'
                );
                const editingId = submitButton.dataset.editingId;
                if (editingId) {
              ...
              const resetKeyButton =
                document.querySelector('#reset-tenant-key');
      ...
      if (this.id === 'tenant-home-button') {
        fetch(ownerURL + '/tenant-home')
          ...
          .then((data) => {
            ...
            const tenantproperty = document.getElementById(
              'tenants-properties-table'
            );
            ...
            if (tenantForm && tenantproperty) {
              ...
              searchInput.addEventListener('input', function () {
                this.value !== ''
                  ? getTenantsPropertiesRequest(1, true) // Search tenant properties
      ...
      if (this.id === 'myreceipt-button') {
        fetch(ownerURL + '/receipts')
          ...
          .then((data) => {
            document.querySelector('.details').innerHTML = data;
            const validReceiptsTable = document.getElementById(
              'valid-receipts-table'
            );
            ...
            if (tenantsPropertiesoption && requireRecieptForm) {
              ...
              const dateTimeInput =
                document.getElementById('receipt-date-hour');
              const now = new Date();
