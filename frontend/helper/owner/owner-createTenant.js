...
function addTenantToTable(tenant) {
  ...
  const formattedDate = new Date(tenant.create_time)
...
function addtenantsPropertiestable(tenantProperty) {
  ...
  const activationLink =
    activateURL + `?key=${tenantProperty.conn_key}&pr_ten=${tenantProperty.id}`;
  ...
  if (copyIcon) {
    copyIcon.addEventListener('click', () => {
      navigator.clipboard
        .writeText('Lien à usage unique: ' + activationLink)
        .then(() => {
  ...
  if (getsold() - registerTenant > 0) {
    ...
    fetch(host + 'create-tenant', {
      ...
      .then((data) => {
        ...
        setNumberOfTenantsProperties(
          parseInt(getNumberOfTenantsProperties(), 10) + 1
        );
