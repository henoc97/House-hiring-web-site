    ...
    fetch(host + 'get-reset-pwd-otp', {
      ...
      .then((response) => {
        if (!response.ok) {
          ...
          if (response.status === 504) {
            ...
            ); // Alert for server unavailability
          }
          if (response.status === 404) {
            messageDiv.textContent =
              'Erreur : ' + "Votre email n'est lié à aucun compte."; // Error message for email not found
      ...
      .then((data) => {
        ...
        if (data.message === 'OTP sent') {
          ...
          messageDiv.textContent =
            'Veuillez vérifier vos courriers pour réinitialiser votre mot de passe.'; // Success message
      ...
      .catch((error) => {
        messageDiv.textContent =
          "Une erreur s'est produite. Veuillez réessayer."; // Error message for general issues
        messageDiv.classList.add('red-message');
        messageDiv.classList.remove('green-message');
      });
  });
