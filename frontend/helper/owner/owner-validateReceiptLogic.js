...
function validateReceiptLogic() {
  ...
  if (receiptData) {
    ...
    const toPay = (receiptData.total_sumpayed * validateReceiptPrice) < 200 ? 200 : (receiptData.total_sumpayed * validateReceiptPrice); // Calculates the amount to pay for receipt validation
    

    receiptData.payment_ids.split(', ').forEach((id) => {
      ...
      if ((getsold() - toPay) > 0) {
        ...
        credentials: 'include',
          body: JSON.stringify({
            "id": id, // Sends the receipt ID in the request body
          })
        })
        .then(response => {
