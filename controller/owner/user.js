...
module.exports.getResetPwdOtp = async (req, res) => {
  ...
  if (mailTest(email)) {
    ...
    var result;
    try {
      const [rows] = await req.connection.query('CALL owner_by_email(?)', [
        email,
      ]);
      result = rows[0];
    } catch (error) {
    ...
    if (result.length > 0 && result[0].id) {
      ...
      res
        .status(404)
        .json({ message: "Votre email n'est associé à aucun compte." });
    }
  } else {
    logger.warn(`404 Not Found: ${req.method} ${req.url}`);
...
module.exports.createUserOwner = async (req, res) => {
  ...
  try {
    const [result] = await req.connection.query('CALL insert_owner(?, ?)', [
      pwd,
      email,
    ]);
...
module.exports.updateSold = async (req, res) => {
  ...
  try {
    const [rows] = await req.connection.query('CALL update_sold(?, ?)', [
      userId,
      spend,
    ]);
    logger.info(`200 OK: ${req.method} ${req.url}`);
...
module.exports.uploadImg = async (req, res) => {
  try {
    ...
    const [rows1] = await req.connection.query('CALL get_old_img_url(?)', [
      req.user.userId,
    ]);
