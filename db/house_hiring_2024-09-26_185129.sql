...
) ENGINE=InnoDB AUTO_INCREMENT=207 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='payment_received_table';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=53 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='Records monthly deductions from owners';
/*!40101 SET character_set_client = @saved_cs_client */;
...
BEGIN
    ...
    WHERE tenant_id = (SELECT tenantid FROM property_tenant WHERE id = property_tenantid);
    ...
    -- Calculer le reste du montant dû
    ...
    SET 
        ...
        rest_amount_due = var_rest_amount_due
    WHERE id = new_id;

    -- Retourner les détails du paiement nouvellement inséré
    SELECT 
        pr.id,
        t.firstname,
        t.lastname,
...
BEGIN
    SELECT 
        ...
        SUM(pr.sumpayed) AS total_sumpayed, 
        SUM(pr.accessory_fees) AS total_accessory_fees,  -- Somme des frais accessoires
        ...
        GROUP_CONCAT(DISTINCT pr.unpaid_months ORDER BY pr.unpaid_months SEPARATOR ', ') AS unpaid_months,
        MAX(pr.rest_amount_due) AS rest_amount_due,
        MAX(pr.isviewedbyowner) AS isviewedbyowner,
    ...
    WHERE
        p.ownerid = ownerid AND pr.payment_state = 0
    GROUP BY 
...
BEGIN
    ...
    FROM
        ...
        JOIN owner o ON o.id = ownerid
    WHERE
        p.ownerid = ownerid AND pr.payment_state = 1 AND pr.isviewedbyowner = 1 ORDER BY pr.monthpayed DESC;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2024-09-26 18:52:22
